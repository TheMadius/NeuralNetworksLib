...
void ChessMV::GameBoardTextUpdate()
{
	...
	if (game->GetInfo().isEnd)
	{
		...
		windowGame->gameTextureAndText.txtInfo->setString((string)"Победитель:" + ((game->GetInfo().winner == ChessGame::Team::Black) ? "Чёрные" : "Белые"));
...
void ChessMV::GameBoardAndСheckersUpdate()
{
